  ...
  if (has_rgb_) {
    ...
    while ((tag = av_dict_get(pFormatCtx_->streams[rgb_frame_id_]->metadata, "", tag, AV_DICT_IGNORE_SUFFIX))) {
      if (std::string(tag->key).find("icam_0") != std::string::npos) {
        ...
        rgb_intrinsics_left_.cx = intrinsics_data[0];
        rgb_intrinsics_left_.cy = intrinsics_data[1];
        rgb_intrinsics_left_.fx = intrinsics_data[2];
        rgb_intrinsics_left_.fy = intrinsics_data[3];
      } else if (std::string(tag->key).find("icam_1") != std::string::npos) {
        std::vector<double> intrinsics_data = SpatialML::String2DoubleVector(tag->value);
        rgb_intrinsics_right_.cx = intrinsics_data[0];
        rgb_intrinsics_right_.cy = intrinsics_data[1];
        rgb_intrinsics_right_.fx = intrinsics_data[2];
        rgb_intrinsics_right_.fy = intrinsics_data[3];
