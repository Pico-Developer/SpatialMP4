...
constexpr uint32_t kHandJointsMagic = 0x544E4A48;       // "HJNT"
constexpr uint32_t kControllerInputMagic = 0x504E4943;  // "CINP"
constexpr size_t kHandJointHeaderBytes = 8;
constexpr size_t kHandJointBytes = 36;
constexpr size_t kControllerInputBytes = 68;

std::string GetMetadata(const AVStream* stream, const char* key) {
  if (!stream) {
    return "";
  }
  AVDictionaryEntry* tag = av_dict_get(stream->metadata, key, nullptr, 0);
  return tag ? std::string(tag->value) : "";
}
...
bool IsAutogeneratedTrackId(const std::string& track_id) {
  return track_id.rfind("timed_", 0) == 0;
}

// SpatialMP4 timed-metadata tracks encode their (kind, track_id) identity in the
// stream handler_name, formatted as "spatialmp4:<kind>:<track_id>". This is used
// in preference to plain av_dict stream metadata (track_id / metadata_kind / ...)
// because the MP4 muxer does not persist arbitrary per-stream metadata, whereas
// the hdlr box handler_name reliably round-trips. Returns {"", ""} when the
// stream is not a spatialmp4 timed-metadata handler.
std::pair<std::string, std::string> ParseSpatialHandler(const AVStream* stream) {
  static const std::string kPrefix = "spatialmp4:";
  const std::string handler = GetMetadata(stream, "handler_name");
  if (handler.rfind(kPrefix, 0) != 0) {
    return {"", ""};
  }
  const std::string rest = handler.substr(kPrefix.size());
...
std::string PosePositionToTrackId(const std::string& pose_position) {
  if (pose_position == "head") return "head";
  if (pose_position == "lctr") return "left_controller";
  if (pose_position == "rctr") return "right_controller";
  if (pose_position == "lhnd") return "left_hand";
  if (pose_position == "rhnd") return "right_hand";
  if (pose_position == "lcin") return "left_controller_input";
  if (pose_position == "rcin") return "right_controller_input";
  return pose_position;
}

std::string TimedMetadataKind(const AVStream* stream) {
  const std::string handler_kind = ParseSpatialHandler(stream).first;
  if (!handler_kind.empty()) {
    return handler_kind;
  }
  const std::string kind = GetMetadata(stream, "metadata_kind");
  if (!kind.empty()) {
    return kind;
  }
  const std::string schema = GetMetadata(stream, "payload_schema");
  const std::string mime = GetMetadata(stream, "mime_type");
...
std::string TimedTrackId(const AVStream* stream, int stream_index) {
  const std::string handler_track = ParseSpatialHandler(stream).second;
  if (!handler_track.empty()) {
    return handler_track;
  }
  std::string track_id = GetMetadata(stream, "track_id");
  if (!track_id.empty()) {
    return track_id;
  }
  track_id = GetMetadata(stream, "pose_track_id");
  if (!track_id.empty()) {
    return track_id;
  }
  track_id = PosePositionToTrackId(GetMetadata(stream, "pose_position"));
  if (!track_id.empty()) {
    return track_id;
  }
  return "timed_" + std::to_string(stream_index);
}
...
Reader::Reader(const std::string& filename, const std::string& log_level)
  ...
  for (unsigned int stream_index_u = 0; stream_index_u < pFormatCtx_->nb_streams; ++stream_index_u) {
    const int stream_index = static_cast<int>(stream_index_u);
    auto type_iter = stream_type_info.find(stream_index);
    if (type_iter == stream_type_info.end()) {
      continue;
    }
    StreamType stream_type = type_iter->second;
    switch (stream_type) {
      ...
      case MEDIA_TYPE_POSE:
        timed_metadata_frame_ids_.push_back(stream_index);
        {
          AVStream* stream = pFormatCtx_->streams[stream_index];
          const std::string kind = TimedMetadataKind(stream);
          ...
          if (kind == "hand_joints") {
            ...
            rigid_pose_stream_ids_[stream_index] = track_id;
            if (track_id == "head" || PosePositionToTrackId(GetMetadata(stream, "pose_position")) == "head") {
              has_pose_ = true;
  ...
  if (!has_pose_ && rigid_pose_stream_ids_.size() == 1) {
    auto only = rigid_pose_stream_ids_.begin();
    pose_frame_id_ = only->first;
    only->second = "head";
    has_pose_ = true;
  } else if (!has_pose_ && !rigid_pose_stream_ids_.empty()) {
    bool all_autogenerated = true;
    int highest_stream_index = -1;
    for (const auto& pair : rigid_pose_stream_ids_) {
      all_autogenerated = all_autogenerated && IsAutogeneratedTrackId(pair.second);
      highest_stream_index = std::max(highest_stream_index, pair.first);
    }
    if (all_autogenerated && highest_stream_index >= 0) {
      pose_frame_id_ = highest_stream_index;
      rigid_pose_stream_ids_[highest_stream_index] = "head";
      has_pose_ = true;
    }
  }
  // get start timestamp
  ...
  if (has_depth_) {
    ...
    // Backward-compatible depth decode path. Legacy captures store depth as
    // uncompressed GRAY16LE (codec_id == NONE / 'raw1') and the packet bytes are
    // the samples themselves, so no decoder is needed. Newer captures encode
    // depth with FFV1 (lossless, intra-only); spin up a decoder for those.
    AVCodecParameters* depth_par = pFormatCtx_->streams[depth_frame_id_]->codecpar;
    if (depth_par->codec_id != AV_CODEC_ID_NONE) {
      depth_codec_ = avcodec_find_decoder(depth_par->codec_id);
      if (!depth_codec_) {
        throw std::runtime_error("Depth codec not found");
      }
      depth_codec_ctx_ = avcodec_alloc_context3(depth_codec_);
      if (!depth_codec_ctx_) {
        throw std::runtime_error("Could not allocate depth codec context");
      }
      if (avcodec_parameters_to_context(depth_codec_ctx_, depth_par) < 0) {
        throw std::runtime_error("Could not copy depth codec parameters");
      }
      if (avcodec_open2(depth_codec_ctx_, depth_codec_, NULL) < 0) {
        throw std::runtime_error("Could not open depth codec");
      }
      depth_frame_av_ = av_frame_alloc();
    }
  }

  for (const auto& pair : rigid_pose_stream_ids_) {
    LoadAllRigidPoseData(pair.first, pair.second, pair.first == pose_frame_id_);
  }
...
Reader::~Reader() {
  ...
  if (depth_codec_ctx_ != nullptr) {
    avcodec_free_context(&depth_codec_ctx_);
  }
  if (depth_frame_av_ != nullptr) {
    av_frame_free(&depth_frame_av_);
  }
}

std::vector<std::string> Reader::ListTimedMetadataTracks() const {
  std::vector<std::string> tracks;
  for (const auto& pair : rigid_pose_tracks_) {
...
std::vector<pose_frame> Reader::GetRigidPoseFrames(const std::string& track_id) const {
  auto it = rigid_pose_tracks_.find(track_id);
  if (it == rigid_pose_tracks_.end()) {
    return {};
  }
  return it->second;
}
...
std::vector<controller_input_frame> Reader::GetControllerInputFrames(const std::string& track_id) const {
  auto it = controller_input_tracks_.find(track_id);
  if (it == controller_input_tracks_.end()) {
    return {};
  }
  return it->second;
}
...
void Reader::LoadAllPoseData(int frame_id) {
  LoadAllRigidPoseData(frame_id, "head", true);
}

void Reader::LoadAllRigidPoseData(int frame_id, const std::string& track_id, bool mirror_to_legacy_head) {
  const AVStream* pose_stream = pFormatCtx_->streams[frame_id];
  AVPacket pkt;
  while (av_read_frame(pFormatCtx_, &pkt) >= 0) {
    if (pkt.stream_index == frame_id) {
      if (pkt.size < static_cast<int>(7 * sizeof(double))) {
        av_packet_unref(&pkt);
        continue;
      }
      ...
      rigid_pose_tracks_[track_id].push_back(pose_frame);
      if (mirror_to_legacy_head) {
...
void Reader::LoadAllHandJointsData(int frame_id, const std::string& track_id) {
  ...
  while (av_read_frame(pFormatCtx_, &pkt) >= 0) {
    if (pkt.stream_index == frame_id) {
      ...
      if (pkt.size < static_cast<int>(kHandJointHeaderBytes) ||
          ReadPod<uint32_t>(data, 0) != kHandJointsMagic ||
          ReadPod<uint16_t>(data, 4) != 1) {
        av_packet_unref(&pkt);
        continue;
      }
      const uint16_t joint_count = ReadPod<uint16_t>(data, 6);
      const size_t expected_size = kHandJointHeaderBytes + static_cast<size_t>(joint_count) * kHandJointBytes;
      if (pkt.size < static_cast<int>(expected_size)) {
        av_packet_unref(&pkt);
        continue;
      }
      hand_joints_frame frame;
      frame.timestamp = pkt.pts * av_q2d(stream->time_base);
      frame.track_id = track_id;
      frame.joints.reserve(joint_count);
      size_t offset = kHandJointHeaderBytes;
      for (uint16_t i = 0; i < joint_count; ++i) {
        hand_joint joint;
        joint.joint_id = ReadPod<uint16_t>(data, offset);
        offset += 2;
        joint.flags = ReadPod<uint16_t>(data, offset);
        offset += 2;
        joint.radius_m = ReadPod<float>(data, offset);
        offset += 4;
        joint.x = ReadPod<float>(data, offset);
        offset += 4;
        joint.y = ReadPod<float>(data, offset);
        offset += 4;
        joint.z = ReadPod<float>(data, offset);
        offset += 4;
        joint.qx = ReadPod<float>(data, offset);
        offset += 4;
        joint.qy = ReadPod<float>(data, offset);
        offset += 4;
        joint.qz = ReadPod<float>(data, offset);
        offset += 4;
        joint.qw = ReadPod<float>(data, offset);
        offset += 4;
...
void Reader::LoadAllControllerInputData(int frame_id, const std::string& track_id) {
  const AVStream* stream = pFormatCtx_->streams[frame_id];
  AVPacket pkt;
  while (av_read_frame(pFormatCtx_, &pkt) >= 0) {
    if (pkt.stream_index == frame_id) {
      const uint8_t* data = pkt.data;
      if (pkt.size < static_cast<int>(kControllerInputBytes) ||
          ReadPod<uint32_t>(data, 0) != kControllerInputMagic ||
          ReadPod<uint16_t>(data, 4) != 1) {
        av_packet_unref(&pkt);
        continue;
      }
      controller_input_frame frame;
      frame.timestamp = pkt.pts * av_q2d(stream->time_base);
      frame.track_id = track_id;
      frame.packet_type = ReadPod<uint16_t>(data, 6);
      frame.controller = ReadPod<uint16_t>(data, 8);
      frame.available_mask = ReadPod<uint64_t>(data, 12);
      frame.pressed_mask = ReadPod<uint64_t>(data, 20);
      frame.touched_mask = ReadPod<uint64_t>(data, 28);
      frame.changed_mask = ReadPod<uint64_t>(data, 36);
      frame.trigger_value = ReadPod<float>(data, 44);
      frame.grip_value = ReadPod<float>(data, 48);
      frame.thumbstick_x = ReadPod<float>(data, 52);
      frame.thumbstick_y = ReadPod<float>(data, 56);
      frame.trackpad_x = ReadPod<float>(data, 60);
      frame.trackpad_y = ReadPod<float>(data, 64);
      controller_input_tracks_[track_id].push_back(frame);
    }
    av_packet_unref(&pkt);
  }
  av_seek_frame(pFormatCtx_, -1, 0, AVSEEK_FLAG_BACKWARD);
...
void Reader::ParseDepthFrame(const AVPacket& pkt, depth_frame& frame_depth, bool raw_head_pose) {
  ...
  if (depth_codec_ctx_ != nullptr) {
    // Compressed depth (FFV1, lossless intra). Decode to GRAY16LE, then copy
    // into a contiguous CV_16UC1 honouring the decoder's linesize before the
    // millimetre->metre scale, so the result is identical to the legacy raw1
    // path bit-for-bit.
    int ret = avcodec_send_packet(depth_codec_ctx_, &pkt);
    if (ret < 0) {
      throw std::runtime_error("Error sending packet to depth decoder");
    }
    ret = avcodec_receive_frame(depth_codec_ctx_, depth_frame_av_);
    if (ret < 0) {
      throw std::runtime_error("Error receiving frame from depth decoder");
    }
    cv::Mat depth_u16(depth_height_, depth_width_, CV_16UC1);
    const int row_bytes = depth_width_ * 2;
    for (int y = 0; y < depth_height_; ++y) {
      std::memcpy(depth_u16.ptr(y), depth_frame_av_->data[0] + static_cast<ptrdiff_t>(y) * depth_frame_av_->linesize[0],
                  static_cast<size_t>(row_bytes));
    }
    depth_u16.convertTo(frame_depth.depth, CV_32FC1, 0.001);  // unit: meter
    av_frame_unref(depth_frame_av_);
  } else {
    // Legacy raw1: the packet payload is the raw GRAY16LE sample grid.
